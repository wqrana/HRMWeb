...
BEGIN
	...
	END
	IF NOT EXISTS (SELECT * FROM [dbo].[EmployeeGroup] WHERE EmployeeGroupName = 'Company Media Admin' And [ClientId]=@ClientId)
	BEGIN
		INSERT [dbo].[EmployeeGroup] ([EmployeeGroupName], [EmployeeGroupDescription], [EmployeeGroupTypeId], [CompanyId], [ClientId], [CreatedBy], [CreatedDate], [DataEntryStatus], [ModifiedBy], [ModifiedDate]) VALUES (N'Company Media Admin', N'Company Media Admin', 8, NULL, @ClientId, 1, GetDate(), 1, NULL, NULL);
	END
END
