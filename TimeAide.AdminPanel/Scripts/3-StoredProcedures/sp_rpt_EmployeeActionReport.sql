...
-- Description:	<Fetch the Employee Action Report data>
-- =============================================
CREATE PROCEDURE [dbo].[sp_rpt_EmployeeActionReport] 
...
BEGIN
	...
	Declare @selectedActionTypeTbl As Table(Id Bigint)
	...
	Insert Into @selectedActionTypeTbl
	Select Id 
	From dbo.fn_splitIntIds(@actionTypeIds,',')

	SELECT distinct empgrm.EmployeeId,empgrm.ShortFullName,empAction.*
	FROM vw_rpt_EmployeeActionInfo empAction
	INNER JOIN vw_rpt_EmployeeGeneralRptMain empgrm ON empgrm.UserInformationId = empAction.UserInformationId
	...
	AND  (empAction.ActionTypeId in (Select * From @selectedActionTypeTbl)
	        OR empAction.ActionTypeId = IIF(@actionTypeIds='', empAction.ActionTypeId,0))
