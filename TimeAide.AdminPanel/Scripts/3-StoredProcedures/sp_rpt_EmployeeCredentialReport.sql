...
-- Description:	<Fetch the Employee Credential Report data>
-- =============================================
CREATE PROCEDURE [dbo].[sp_rpt_EmployeeCredentialReport] 
...
BEGIN
	...
	Declare @selectedCredentialTbl As Table(Id Bigint)
	...
	Insert Into @selectedCredentialTbl
	Select Id 
	From dbo.fn_splitIntIds(@credentialIds,',')

	SELECT distinct empgrm.EmployeeId,empgrm.ShortFullName,empCred.*
	FROM vw_rpt_EmployeeCredentialInfo empCred
	INNER JOIN vw_rpt_EmployeeGeneralRptMain empgrm ON empgrm.UserInformationId = empCred.UserInformationId
	...
	AND  (empCred.CredentialId in (Select * From @selectedCredentialTbl)
	        OR empCred.CredentialId = IIF(@credentialIds='', empCred.CredentialId,0))
