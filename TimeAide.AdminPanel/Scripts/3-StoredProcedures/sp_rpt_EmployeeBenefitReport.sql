...
-- Description:	<Fetch the Employee Benefit Report data>
-- =============================================
CREATE PROCEDURE [dbo].[sp_rpt_EmployeeBenefitReport] 
...
BEGIN
	...
	Declare @selectedBenefitTbl As Table(Id Bigint)
	...
	Insert Into @selectedBenefitTbl
	Select Id 
	From dbo.fn_splitIntIds(@benefitIds,',')

	SELECT distinct empgrm.EmployeeId,empgrm.ShortFullName,empBenefit.*
	FROM vw_rpt_EmployeeBenefitInfo empBenefit
	INNER JOIN vw_rpt_EmployeeGeneralRptMain empgrm ON empgrm.UserInformationId = empBenefit.UserInformationId
	...
	AND  (empBenefit.BenefitId in (Select * From @selectedBenefitTbl)
	        OR empBenefit.BenefitId = IIF(@benefitIds='', empBenefit.BenefitId,0))
