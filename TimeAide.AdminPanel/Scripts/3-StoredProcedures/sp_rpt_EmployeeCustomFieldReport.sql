...
-- Description:	<Fetch the Employee Custom Field Report data>
-- =============================================
CREATE PROCEDURE [dbo].[sp_rpt_EmployeeCustomFieldReport] 
...
BEGIN
	...
	Declare @selectedCustomFieldTbl As Table(Id Bigint)
	...
	Insert Into @selectedCustomFieldTbl
	Select Id 
	From dbo.fn_splitIntIds(@customFieldIds,',')

	SELECT distinct empgrm.EmployeeId,empgrm.ShortFullName,empCustField.*
	FROM vw_rpt_EmployeeCustomFieldInfo empCustField
	INNER JOIN vw_rpt_EmployeeGeneralRptMain empgrm ON empgrm.UserInformationId = empCustField.UserInformationId
	...
	AND  (empCustField.CustomFieldId in (Select * From @selectedCustomFieldTbl)
	        OR empCustField.CustomFieldId = IIF(@customFieldIds='', empCustField.CustomFieldId,0))
