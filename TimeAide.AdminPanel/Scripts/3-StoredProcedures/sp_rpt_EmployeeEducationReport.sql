...
-- =============================================
CREATE PROCEDURE [dbo].[sp_rpt_EmployeeEducationReport] 
...
BEGIN
	...
	Declare @selectedDegreeTbl As Table(Id Bigint)
	---Employee List
	...
	Insert Into @selectedDegreeTbl
	Select Id 
	From dbo.fn_splitIntIds(@degreeIds,',')	 

	SELECT distinct empgrm.EmployeeId,empgrm.ShortFullName,empEduInfo.*
	FROM vw_rpt_EmployeeEducationInfo empEduInfo
	INNER JOIN vw_rpt_EmployeeGeneralRptMain empgrm ON empgrm.UserInformationId = empEduInfo.UserInformationId
	...
	AND  (empEduInfo.DegreeId in (Select * From @selectedDegreeTbl)
	        OR empEduInfo.DegreeId = IIF(@degreeIds='', empEduInfo.DegreeId,0))
