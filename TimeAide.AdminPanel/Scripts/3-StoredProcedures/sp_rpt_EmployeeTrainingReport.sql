...
-- Description:	<Fetch the Employee Education Report data>
-- =============================================
CREATE PROCEDURE [dbo].[sp_rpt_EmployeeTrainingReport] 
...
BEGIN
	...
	Declare @selectedTrainingTbl As Table(Id Bigint)
	---Employee List
	...
	Insert Into @selectedTrainingTbl
	Select Id 
	From dbo.fn_splitIntIds(@trainingIds,',')	 

	SELECT distinct empgrm.EmployeeId,empgrm.ShortFullName,empTrainingInfo.*
	FROM vw_rpt_EmployeeTrainingInfo empTrainingInfo
	INNER JOIN vw_rpt_EmployeeGeneralRptMain empgrm ON empgrm.UserInformationId = empTrainingInfo.UserInformationId
	...
	AND  (empTrainingInfo.TrainingId in (Select * From @selectedTrainingTbl)
	        OR empTrainingInfo.TrainingId = IIF(@trainingIds='', empTrainingInfo.TrainingId,0))
