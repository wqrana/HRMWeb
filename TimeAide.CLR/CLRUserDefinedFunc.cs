...
public partial class UserDefinedFunctions
{
    ...
    public static SqlString fn_CLREnDecriptSSN(string ssnStr, string actionType)
    {
        string returnSSN = "";
        if (ssnStr != "")
        {
            if (actionType == "E")
            {
                returnSSN = Encryption.Encrypt(ssnStr);
            }
            else if (actionType == "D")
            {
                returnSSN = Encryption.Decrypt(ssnStr);
            }

        }
        // Put your code here
        return new SqlString(returnSSN);
    }
}
