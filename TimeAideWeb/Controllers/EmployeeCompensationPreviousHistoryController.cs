...
{
    public class EmployeeCompensationPreviousHistoryController : TimeAideWebControllers<EmployeeCompensationPreviousHistory>
    {
        ...
        {
            base.OnCreate();
            ViewBag.CompanyCompensationId = new SelectList(db.GetAllByCompany<CompanyCompensation>(SessionHelper.CompanyId, SessionHelper.ClientId), "Id", "CompensationName");
            var lookupfield= new List<SelectListItem>(){
                                            new SelectListItem() { Text = "Yes", Value = "1" },
                                            new SelectListItem() { Text = "No", Value = "0"}
                                            };

            ViewBag.GLLookupField = new SelectList(lookupfield, "Value", "Text");
        ...
        public ActionResult AddEmployeeCompensation(EmployeeCompensation employeeCompensation)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    ModelState.AddModelError("CompanyCompensationId", "Compensation cannot be duplicate in active list.");
                }
            }
            if (ModelState.IsValid)
            {
        ...
        public ActionResult Deactivate(int? id)
        {
            ...
            ViewBag.TerminationEligibilityId = new SelectList(db.GetAll<TerminationEligibility>(SessionHelper.SelectedClientId), "Id", "TerminationEligibilityName");
            ViewBag.TerminationReasonId = new SelectList(db.GetAll<TerminationReason>(SessionHelper.SelectedClientId), "Id", "TerminationReasonName");
            ViewBag.TerminationTypeId = new SelectList(db.GetAll<TerminationType>(SessionHelper.SelectedClientId), "Id", "TerminationTypeName");
            // ViewBag.ApprovedById = new SelectList(db.GetAll<UserInformation>(SessionHelper.SelectedClientId).Where(w => w.EmployeeStatusId == 1), "Id", "ShortFullName");
            //Employee superviser
            ViewBag.ApprovedById = new SelectList(db.EmployeeSupervisor.Where(e => e.ClientId == SessionHelper.SelectedClientId && e.EmployeeUserId == id).Select(e => e.SupervisorUser), "Id", "ShortFullName");
        ...
        public JsonResult Deactivate(Employment model)
        {
            ...
            {
                status = "Error";
                message = "Current Hiring detail is not available";
            }
            else
            {
                try
                {
                    ...
                    employmentEntity.UserInformation.EmployeeStatusId = 3;
                    ...
                    db.SaveChanges();
                    userStatus = db.EmployeeStatus.Select(s => new { id = s.Id, name = s.EmployeeStatusName }).Where(w => w.id == 3).FirstOrDefault();
                }
        ...
        public ActionResult EmployeeTransfer(int? id)
        {
            ...
            var userInformationEntity = db.UserInformation.Where(w => w.Id == model.UserInformationId).FirstOrDefault();
            model.CompanyId = userInformationEntity.CompanyId;
            model.ToCompanyList = db.GetAll<Company>(SessionHelper.SelectedClientId).Select(s => new { id = s.Id, text = s.CompanyName }).Where(w => w.id != model.CompanyId);
        ...
        private bool EmployeeTransferUpdateContactInfo(UserInformation fromUser, UserInformation toUser)
        {
            ...
            if (fromEmployeeContactInfo != null)
            {
                ...
                else
                {
                    ...
                    toEmployeeContactInfo.LoginEmail = fromEmployeeContactInfo.LoginEmail;
