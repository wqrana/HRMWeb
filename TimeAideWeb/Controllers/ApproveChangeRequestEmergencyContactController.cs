...
{
    public class ApproveChangeRequestEmergencyContactController : TimeAideWebSecurotyBaseControllers
    {
        public ApproveChangeRequestEmergencyContactController() : base("ChangeRequestEmergencyContact")
        {
        }
        // GET: ChangeRequestAddress
        public ActionResult ApproveChangeRequestEmergencyContact(int id)
        {
            try
            {
                AllowAdd();
        ...
        [HttpPost]
        public ActionResult ApproveChangeRequestEmergencyContact(ChangeRequestEmergencyContact model)
        {
            try
            {
                AllowAdd();
                //db.Entry(ChangeRequestAddress).State = EntityState.Modified;
                ChangeRequestEmergencyContact changeRequest = db.ChangeRequestEmergencyContact.FirstOrDefault(i => i.Id == model.Id);
                ...
                WorkflowTriggerRequest workflowTriggerRequest = db.WorkflowTriggerRequest.FirstOrDefault(t => t.ChangeRequestEmergencyContactId == changeRequest.Id);
                var detail = WorkflowService.GetNextWorkflowLevel<ChangeRequestEmergencyContact>(db, workflowTriggerRequest, changeRequest);
