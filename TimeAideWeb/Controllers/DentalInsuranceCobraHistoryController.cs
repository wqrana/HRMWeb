...
{
    public class DentalInsuranceCobraHistoryController : TimeAideWebControllers<DentalInsuranceCobraHistory>
    {
        // GET: HealthInsuranceCobraHistory
        
        public ActionResult IndexByUserInsurance(int id)
        {
            var model = db.DentalInsuranceCobraHistory.Where(w => w.DataEntryStatus == 1
                                                        && w.EmployeeDentalInsurance.UserInformationId == id
        ...
        {
            var model = db.DentalInsuranceCobraHistory.Where(w => w.Id == id).FirstOrDefault();
        ...
        [HttpPost]
        public JsonResult CreateEdit(DentalInsuranceCobraHistory model)
        {
            string status = "Success";
            string message = "Successfully Added/Updated!";
            DentalInsuranceCobraHistory employeeInsuranceCobraHistoryEntity = null;
            try
            {
                if (model.Id == 0)
                {
                    employeeInsuranceCobraHistoryEntity = new DentalInsuranceCobraHistory();
                    employeeInsuranceCobraHistoryEntity.EmployeeDentalInsuranceId = model.EmployeeDentalInsuranceId;
                    db.DentalInsuranceCobraHistory.Add(employeeInsuranceCobraHistoryEntity);
                }
                else
                {
                    employeeInsuranceCobraHistoryEntity = db.DentalInsuranceCobraHistory.Find(model.Id);
        ...
        public JsonResult ConfirmDelete(int id)
        {
            ...
            var employeeInsuranceCobraHistorEntity = db.DentalInsuranceCobraHistory.Find(id);
            try
            {
